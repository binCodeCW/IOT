...
namespace WHC.ContactBook.DALSQL
{
    ...
    /// 通讯录分组
    /// </summary>
	public class AddressGroup : BaseDALSQL<AddressGroupInfo>, IAddressGroup
	{
		#region 对象实例及构造函数
		...
		public AddressGroup() : base("TB_AddressGroup","ID")
		{
            this.SortField = "SEQ";
        ...
        {
            string sql = string.Format(@"Select t.* From {0} t inner join TB_AddressGroup_Address m 
            ON t.ID = m.Group_ID Where m.Address_ID = '{1}' ", tableName, contactId);
