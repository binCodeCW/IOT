...
using WHC.ContactBook.IDAL;

namespace WHC.ContactBook.DALSQL
{
    /// <summary>
    /// 通讯录联系人
    /// </summary>
    public class Address : BaseDALSQL<AddressInfo>, IAddress
    {
        #region 对象实例及构造函数
        ...
        public List<AddressInfo> FindByGroupName(string ownerUser, string groupName, PagerInfo pagerInfo = null)
        {
            ...
            {
                sql = string.Format("Select t.* from {0} t where ID not In (select Address_ID from TB_AddressGroup_Address) {1}", tableName, condition);
            }
            else
            {
                string subSql = string.Format("select ID from TB_AddressGroup g where g.Name ='{0}' ", groupName);
                sql = string.Format(@"select t.* from {0} t inner join TB_AddressGroup_Address m on t.ID = m.Address_ID 
                where m.Group_ID in ({1}) {2} ", tableName, subSql, condition);
            }
        ...
        public bool ModifyAddressGroup(string contactId, List<string> groupIdList)
        {
            ...
            if (trans != null)
            {
                string sql = string.Format("Delete from TB_AddressGroup_Address where Address_ID='{0}' ", contactId);
                base.SqlExecute(sql, trans);

                foreach (string groupId in groupIdList)
                {
                    sql = string.Format("Insert into TB_AddressGroup_Address(Address_ID, Group_ID) values('{0}', '{1}') ", contactId, groupId);
                    base.SqlExecute(sql, trans);
                }
