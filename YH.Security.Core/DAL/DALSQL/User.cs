...
namespace YH.Security.DALSQL
{
    ...
    /// 系统用户信息
    /// </summary>
    public class User : BaseDALSQL<UserInfo>, IUser
    {
        private const string SimpleUser_ColumnString = "ID,Name,Password,FullName,HandNo,MobilePhone,Email,Dept_ID,Company_ID";
        ...
        protected override Hashtable GetHashByEntity(UserInfo obj)
        {
            ...
            //hash.Add("OpenId", info.OpenId);
            //hash.Add("UnionId", info.UnionId);
            hash.Add("Status", info.Status);
            hash.Add("SubscribeWechat", info.SubscribeWechat);
            hash.Add("DeptPermission", info.DeptPermission);
            //hash.Add("CorpUserId", info.CorpUserId);
            hash.Add("CorpStatus", info.CorpStatus);
        ...
        public override Dictionary<string, string> GetColumnNameAlias()
        {
            ...
            dict.Add("OpenId", "微信绑定的OpenId");
            dict.Add("UnionId", "微信多平台应用下的统一ID");
            dict.Add("Status", "公众号状态");
            dict.Add("SubscribeWechat", "公众号");
            dict.Add("DeptPermission", "科室权限");
            dict.Add("CorpUserId", "企业微信UserID");
            dict.Add("CorpStatus", "企业微信状态");
            #endregion
        ...
        {
            return "HandNo,Name,FullName,Title,MobilePhone,OfficePhone,Email,Gender,QQ,AuditStatus,IsExpire,ExpireDate,Note";
        }
        ...
        private List<SimpleUserInfo> FillSimpleUsers(string sql)
        {
            ...
            using (IDataReader reader = db.ExecuteReader(command))
            {
                ...
                while (reader.Read())
                {
                    ...
                    info.Dept_ID = dr.GetString("Dept_ID");
                    info.Company_ID = dr.GetString("Company_ID");
                    list.Add(info);
                }
            }
            return list;
        ...
        public List<SimpleUserInfo> FindSimpleUsers(string condition)
        {
            ...
            //串连条件语句为一个完整的Sql语句
            string sql = string.Format("Select {1} From {0} Where Deleted = 0 ", tableName, SimpleUser_ColumnString);
        ...
        public List<SimpleUserInfo> GetSimpleUsersByOU(int ouID)
        {
            ...
            Inner Join T_ACL_OU_User ON {0}.ID=User_ID Where Deleted = 0 AND T_ACL_OU_User.OU_ID = {1}", tableName, ouID, SimpleUser_ColumnString);
            return FillSimpleUsers(sql);
        }
        ...
        {
            string sql = string.Format(@"Select {2} From {0} 
            INNER JOIN T_ACL_User_Role ON {0}.ID=User_ID Where Deleted = 0 AND T_ACL_User_Role.Role_ID ={1} ", tableName, roleID, SimpleUser_ColumnString);
        ...
        public bool UpdatePersonImageBytes(UserImageType imagetype, int userId, byte[] imageBytes)
        {
            ...
            Database db = CreateDatabase();
            DbCommand dbCommand = db.GetSqlStringCommand(sql);
            db.AddInParameter(dbCommand, ParameterPrefix + "image", DbType.Binary, imageBytes);
        ...
        public bool UpdateUserLoginData(int id, string ip, string macAddr)
        {
            ...
            sql = string.Format("Update {0} Set CurrentLoginIP='{1}',CurrentMacAddress='{2}', CurrentLoginTime={4}CurrentLoginTime Where ID = {3}", tableName, ip, macAddr, id, ParameterPrefix);
            dbCommand = db.GetSqlStringCommand(sql);
            db.AddInParameter(dbCommand, ParameterPrefix + "CurrentLoginTime", DbType.DateTime, DateTime.Now);
            return db.ExecuteNonQuery(dbCommand) > 0;
        }
