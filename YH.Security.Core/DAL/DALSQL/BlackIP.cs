...
namespace YH.Security.DALSQL
{
    ...
    public class BlackIP : BaseDALSQL<BlackIPInfo>, IBlackIP
    {
        ...
        {
            return "Name,AuthorizeType,Forbid,IPStart,IPEnd,Note,Creator,CreateTime";
        }
        ...
        {
            string sql = string.Format("INSERT INTO T_ACL_BLACKIP_USER(User_ID, BLACKIP_ID) VALUES({0}, '{1}')", userID, blackID);
            SqlExecute(sql);
        }

        public void RemoveUser(int userID, string blackID)
        {
            string sql = string.Format("DELETE FROM T_ACL_BLACKIP_USER WHERE User_ID={0} AND BLACKIP_ID='{1}'", userID, blackID);
            SqlExecute(sql);
        }
