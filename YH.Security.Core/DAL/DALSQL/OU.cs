...
namespace YH.Security.DALSQL
{
    ...
    public class OU : BaseDALSQL<OUInfo>, IOU
    {
        ...
        {
            string sql = string.Format("INSERT INTO T_ACL_OU_User(User_ID, OU_ID) VALUES({0},{1})", userID, ouID);
            SqlExecute(sql);
        }

        public void RemoveUser(int userID, int ouID)
        {
            string sql = string.Format("DELETE FROM T_ACL_OU_User WHERE User_ID={0} AND OU_ID={1}", userID, ouID);
            SqlExecute(sql);
        }
