...
namespace YH.Framework.Commons
{
    ...
    public class DataTableHelper
    {
        ...
        /// <summary>
        /// 获取Excel文件内容并把它转换为DataTable对象
        /// </summary>
        /// <returns></returns>
        public static DataTable GetExcel()
        {
            OpenFileDialog openFileDialog = new OpenFileDialog();
            openFileDialog.Title = "打开Excel";
            openFileDialog.Filter = "Excel文档(*.xls;*.xlsx)|*.xls;*.xlsx|所有文件(*.*)|*.*";
            DataTable result;
            if (openFileDialog.ShowDialog() == DialogResult.OK)
            {
                string excelConnectionString = string.Format("Provider = Microsoft.Jet.OLEDB.4.0 ; Data Source = '{0}'; Extended Properties=Excel 8.0", 
                ...
                using (OleDbConnection conn = new OleDbConnection(excelConnectionString))
                {
                    ...
                    {
                        SelectCommand = new OleDbCommand(sqlString, conn)
                    }.Fill(dtExcel);
                }
                result = dtExcel;
            }
        ...
        /// <returns></returns>
        public static DataTable GetExcel(string excelFileName)
        {
