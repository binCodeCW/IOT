    ...
    public abstract class AbstractBaseDAL<T> where T : BaseEntity, new()
	{
        ...
        protected virtual void OperationLogOfUpdate(Hashtable recordField, object id, DbTransaction trans = null)
        {
            if (OnOperationLog != null)
            {
                ...
                {
                    //如果对比结果非空，在备注里面增加主键信息
                    StringBuilder sb = new StringBuilder(note);
                    if (!recordField.ContainsKey(PrimaryKey))
                    {
                        sb.AppendLine();
                        sb.AppendLine(string.Format("{0}:{1}", PrimaryKey, id));
                    }
                    var keyNote = sb.ToString();

                    OnOperationLog(userId, this.tableName, operationType, keyNote, trans);
                }
            }
        }
