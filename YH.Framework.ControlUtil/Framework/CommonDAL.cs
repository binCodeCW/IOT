...
namespace YH.Framework.ControlUtil
{
    /// <summary>
	...
	{        
        ...
        /// 数据库类型，默认为Sqlserver
        /// </summary>
        public string DbComponentType { get; set; }
        ...
        public CommonDAL()
		{
            ...
            this.DbComponentType = "sqlserver";//默认数据库类型
        }
        ...
        public CommonDAL(string tableName) : this(tableName, null)
        {
		...
		/// </summary>
		/// <param name="tableName">表名</param>
		/// <param name="this.PrimaryKey">表主键</param>
        /// <param name="dbType">数据库类型，如果为空从配置文件里面获取ComponentDbType的键值</param>
        public CommonDAL(string tableName, string primaryKey, string dbType = "sqlserver")
            : this()
		{
            this.TableName = tableName;
			this.PrimaryKey = primaryKey;
            this.SortField = primaryKey;//默认为主键排序
            this.DbComponentType = dbType;

            InitDbParameter(dbType);
        }
        ...
        private void InitDbParameter(string dbType)
        {
        ...
        /// <param name="dbType">数据库类型</param>
        public virtual void SetDbConfigName(string dbConfigName, string dbType = "sqlserver")
        {
        ...
        {
            Database db = null;
            if (string.IsNullOrEmpty(this.DbConfigName))
            {
                db = DatabaseFactory.CreateDatabase();
            }
            else
            {
                db = DatabaseFactory.CreateDatabase(this.DbConfigName);
            }
            return db;
        }
        ...
        public virtual bool Insert(Hashtable recordField, string targetTable, DbTransaction trans)
        {
            ...
            foreach (string field in recordField.Keys)
            {
                ...
                vals += string.Format("{0}{1},", this.ParameterPrefix, field);
            }
        ...
        public virtual bool PrivateUpdate(object id, Hashtable recordField, string targetTable, DbTransaction trans)
        {
            ...
            {
                throw new ArgumentNullException("this.PrimaryKey", "表的主键字段this.PrimaryKey不能为空！");
            }
            ...
            try
            {
                ...
                foreach (string field in recordField.Keys)
                {
                    setValue += string.Format("{0} = {1}{2},", GetSafeFileName(field), this.ParameterPrefix, field);
                }
                string sql = string.Format("UPDATE {0} SET {1} WHERE {2} = {3}{2} ",
                    targetTable, setValue.Substring(0, setValue.Length - 1), this.PrimaryKey, this.ParameterPrefix);
                Database db = CreateDatabase();
                DbCommand command = db.GetSqlStringCommand(sql);
                ...
                foreach (string field in recordField.Keys)
                {
                    ...
                    if (field.Equals(this.PrimaryKey, StringComparison.OrdinalIgnoreCase))
                    {
                        foundID = true;
                    }
                }
                ...
                if (!foundID)
                {
                    db.AddInParameter(command, ParameterPrefix + this.PrimaryKey, TypeToDbType(id.GetType()), id);
                }
        ...
        private DataTable PrivateFindByID(object key, DbTransaction trans = null)
        {
            ...
            string sql = string.Format("Select {0} From {1} Where ({2} = {3}ID)", this.SelectedFields, this.TableName, this.PrimaryKey, this.ParameterPrefix);
        ...
        public virtual DataTable FindWithPager(string condition, PagerInfo info, string fieldToSort, bool desc, DbTransaction trans = null)
        {
            ...
            PagerHelper helper = new PagerHelper(this.TableName, this.SelectedFields, fieldToSort,
                info.PageSize, info.CurrenetPageIndex, desc, condition);

            string countSql = helper.GetPagingSql(true, this.DbComponentType);
            string strCount = SqlValueList(countSql);
            info.RecordCount = Convert.ToInt32(strCount);

            string dataSql = helper.GetPagingSql(false, this.DbComponentType);
        ...
        public virtual DataTable GetAllToDataTable(string orderBy, DbTransaction trans = null)
        {
            ...
            string sql = string.Format("Select {0} From {1} ", this.SelectedFields, this.TableName);
            ...
            {
                sql += string.Format(" Order by {0} {1}", GetSafeFileName(this.SortField), this.IsDescending ? "DESC" : "ASC");
            }
        ...
        public List<string> GetFieldListByCondition(string fieldName, string condition, DbTransaction trans = null)
        {
            ...
            string sql = string.Format("Select distinct {0} From {1} ", safeFieldName, this.TableName);
            if(!string.IsNullOrEmpty(condition))
            {
                sql += string.Format(" Where {0} ",  condition);
            }
        ...
        public virtual int GetRecordCount(string condition, DbTransaction trans = null)
        {
            ...
            string sql = string.Format("Select Count(*) from {0} WHERE {1} ", this.TableName, condition);
        ...
        public virtual bool IsExistKey(Hashtable recordTable, DbTransaction trans = null)
        {
            ...
            {
                fields += string.Format(" {0} = {1}{2} AND", GetSafeFileName(field), this.ParameterPrefix, field);
            }
            fields = fields.Substring(0, fields.Length - 3);//除去最后的AND

            string sql = string.Format("SELECT COUNT(*) FROM {0} WHERE {1}", this.TableName, fields);
            Database db = CreateDatabase();
            DbCommand command = db.GetSqlStringCommand(sql);
            foreach (string field in recordTable.Keys)
            {
        ...
        public virtual int GetMaxID(DbTransaction trans = null)
		{
			...
			string sql = string.Format("SELECT MAX({0}) AS MaxID FROM {1}", this.PrimaryKey, this.TableName);
        ...
        public virtual string GetFieldValue(object key, string fieldName, DbTransaction trans = null)
        {
            ...
            string sql = string.Format("Select {0} FROM {1} WHERE {2} ", fieldName, this.TableName, condition);

            Database db = CreateDatabase();
            DbCommand command = db.GetSqlStringCommand(sql);
            db.AddInParameter(command, ParameterPrefix + this.PrimaryKey, TypeToDbType(key.GetType()), key);
        ...
        public virtual DataTable GetFieldTypeList()
        {
            ...
            DataTable schemaTable = GetReaderSchema(this.TableName);
            if (schemaTable != null)
            {
        ...
        public virtual DataTable GetReportData(string fieldName, string condition)
        {
            ...
            string sql = string.Format("select {0} as argument, count(*) as datavalue from {1} {2} group by {0} order by count(*) desc", fieldName, this.TableName, where);
