...
{
    public class CostController : BusinessController<Cost, CostInfo>
    {
        ...
        //导入或导出的字段列表   
        string columnString = "费用类型,申请事由,费用金额,申请单编号,申请单日期,申请部门,备注信息,附件组别ID,创建人,创建时间";
        ...
        public ActionResult GetExcelData(string guid)
        {
            ...
            List<CostInfo> list = new List<CostInfo>();
            ...
            if (table != null)
            {
                ...
                foreach (DataRow dr in table.Rows)
                {
                    ...
                    info.Category = dr["费用类型"].ToString();
        ...
        /// <returns></returns>
        public ActionResult SaveExcelData(List<CostInfo> list)
        {
            CommonResult result = new CommonResult();
            if (list != null && list.Count > 0)
            {
                ...
                var trans = BLLFactory<Cost>.Instance.CreateTransaction();
                if (trans != null)
                {
                    try
                    {
                        //int seq = 1;
                        foreach (CostInfo detail in list)
                        {
                            //detail.Seq = seq++;//增加1
                            /*
							...
							*/
                            ...
                            BLLFactory<Cost>.Instance.Insert(detail, trans);
                        }
                        trans.Commit();
                        result.Success = true;
                    }
        ...
        public ActionResult Export()
        {
            ...
            string filePath = string.Format("/GenerateFiles/{0}/Cost.xls", CurrentUser.Name);
        ...
        #region 写入数据前修改部分属性
        protected override void OnBeforeInsert(CostInfo info)
        {
        ...
        protected override void OnBeforeUpdate(CostInfo info)
        {
            //子类对参数对象进行修改
            info.Editor = CurrentUser.ID.ToString();
            info.EditTime = DateTime.Now;
        ...
        /// <returns></returns>
        private List<ExpandoObject> ConvertObjectList(List<CostInfo> list)
        {
            //如果需要修改字段显示，则参考下面代码处理
            List<ExpandoObject> objList = new List<ExpandoObject>();
            foreach (CostInfo info in list)
            {
                ...
                //参考转义代码
                //obj.Name = BLLFactory<Cost>.Instance.GetNameByID(info.ID);
        ...
        public ActionResult FindByApplyId(string applyId)
        {
            ...
            var result = BLLFactory<Cost>.Instance.FindByApplyId(applyId);
            return ToJsonContent(result);
        }
        /// <summary>
        /// 删除多个ID的记录
            ...
            try
            {
                if (!string.IsNullOrEmpty(ids))
                {
                    ...
                    foreach (string strId in idArray)
                    {
                        ...
                        {
                            var info = BLLFactory<Cost>.Instance.FindByID(strId);
                            if (info != null)
                            {
                                //删除关联表单数据，包括业务表单数据
