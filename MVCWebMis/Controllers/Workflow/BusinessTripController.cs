...
{
    public class BusinessTripController : BusinessController<BusinessTrip, BusinessTripInfo>
    {
        public BusinessTripController() : base()
        {
        }
        ...
        //导入或导出的字段列表
        string columnString = "出差事由,出差地点,开始时间,结束时间,出差时长,附件组别ID,申请单编号,申请单日期,申请部门,备注信息,创建人,创建时间";
        ...
        public ActionResult GetExcelData(string guid)
        {
            ...
            List<BusinessTripInfo> list = new List<BusinessTripInfo>();
            ...
            if (table != null)
            {
                ...
                foreach (DataRow dr in table.Rows)
                {
                    ...
                    info.Reason = dr["出差事由"].ToString();
                    info.Location = dr["出差地点"].ToString();
                    ...
                    info.Duration = dr["出差时长"].ToString();
        ...
        /// <returns></returns>
        public ActionResult SaveExcelData(List<BusinessTripInfo> list)
        {
            CommonResult result = new CommonResult();
            if (list != null && list.Count > 0)
            {
                ...
                var trans = BLLFactory<BusinessTrip>.Instance.CreateTransaction();
                if (trans != null)
                {
                    try
                    {
                        //int seq = 1;
                        foreach (BusinessTripInfo detail in list)
                        {
                            //detail.Seq = seq++;//增加1
                            /*
							...
							*/
                            ...
                            BLLFactory<BusinessTrip>.Instance.Insert(detail, trans);
                        }
                        trans.Commit();
                        result.Success = true;
                    }
        ...
        public ActionResult Export()
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                dr = datatable.NewRow();
                dr["序号"] = j++;
                dr["出差事由"] = list[i].Reason;
                dr["出差地点"] = list[i].Location;
                dr["开始时间"] = list[i].StartTime;
                dr["结束时间"] = list[i].EndTime;
                dr["出差时长"] = list[i].Duration;
            ...
            string filePath = string.Format("/GenerateFiles/{0}/BusinessTrip.xls", CurrentUser.Name);
        ...
        #region 写入数据前修改部分属性
        protected override void OnBeforeInsert(BusinessTripInfo info)
        {
        ...
        protected override void OnBeforeUpdate(BusinessTripInfo info)
        {
            //子类对参数对象进行修改
            info.Editor = CurrentUser.ID.ToString();
            info.EditTime = DateTime.Now;
        ...
        public override ActionResult FindWithPager()
        {
            ...
            List<BusinessTripInfo> list = null;
        ...
        /// <returns></returns>
        private List<ExpandoObject> ConvertObjectList(List<BusinessTripInfo> list)
        {
            //如果需要修改字段显示，则参考下面代码处理
            List<ExpandoObject> objList = new List<ExpandoObject>();
            foreach (BusinessTripInfo info in list)
            {
                ...
                //参考转义代码
                //obj.Name = BLLFactory<BusinessTrip>.Instance.GetNameByID(info.ID);
        ...
        public ActionResult FindByApplyId(string applyId)
        {
            ...
            var result = BLLFactory<BusinessTrip>.Instance.FindByApplyId(applyId);
            return ToJsonContent(result);
        }
            ...
            try
            {
                if (!string.IsNullOrEmpty(ids))
                {
                    ...
                    foreach (string strId in idArray)
                    {
                        ...
                        {
                            var info = BLLFactory<BusinessTrip>.Instance.FindByID(strId);
                            if (info != null)
                            {
                                //删除关联表单数据，包括业务表单数据
