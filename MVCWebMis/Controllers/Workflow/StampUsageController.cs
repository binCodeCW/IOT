...
{
    public class StampUsageController : BusinessController<StampUsage, StampUsageInfo>
    {
        public StampUsageController() : base()
        {
        }
        ...
        //导入或导出的字段列表   
        string columnString = "用章事由,印章类型,文件名称,文件份数,用印日期,申请单编号,申请单日期,申请部门,备注信息,附件组别ID,创建人,创建时间";
        ...
        public ActionResult GetExcelData(string guid)
        {
            ...
            List<StampUsageInfo> list = new List<StampUsageInfo>();
            ...
            if (table != null)
            {
                ...
                foreach (DataRow dr in table.Rows)
                {
                    ...
                    info.Reason = dr["用章事由"].ToString();
                    info.StampType = dr["印章类型"].ToString();
                    info.FileName = dr["文件名称"].ToString();
                    info.FileCopies = dr["文件份数"].ToString();
                    converted = DateTime.TryParse(dr["用印日期"].ToString(), out dt);
                    if (converted && dt > dtDefault)
                    {
                        info.StampUseDate = dt;
                    }
        ...
        /// <returns></returns>
        public ActionResult SaveExcelData(List<StampUsageInfo> list)
        {
            CommonResult result = new CommonResult();
            if (list != null && list.Count > 0)
            {
                ...
                var trans = BLLFactory<StampUsage>.Instance.CreateTransaction();
                if (trans != null)
                {
                    try
                    {
                        //int seq = 1;
                        foreach (StampUsageInfo detail in list)
                        {
                            //detail.Seq = seq++;//增加1
                            /*
							...
							*/
                            ...
                            BLLFactory<StampUsage>.Instance.Insert(detail, trans);
                        }
                        trans.Commit();
                        result.Success = true;
                    }
        ...
        public ActionResult Export()
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                dr = datatable.NewRow();
                dr["序号"] = j++;
                dr["用章事由"] = list[i].Reason;
                dr["印章类型"] = list[i].StampType;
                dr["文件名称"] = list[i].FileName;
                dr["文件份数"] = list[i].FileCopies;
                dr["用印日期"] = list[i].StampUseDate;
            ...
            string filePath = string.Format("/GenerateFiles/{0}/StampUsage.xls", CurrentUser.Name);
        ...
        #region 写入数据前修改部分属性
        protected override void OnBeforeInsert(StampUsageInfo info)
        {
        ...
        protected override void OnBeforeUpdate(StampUsageInfo info)
        {
            //子类对参数对象进行修改
            info.Editor = CurrentUser.ID.ToString();
            info.EditTime = DateTime.Now;
        ...
        /// <returns></returns>
        private List<ExpandoObject> ConvertObjectList(List<StampUsageInfo> list)
        {
            //如果需要修改字段显示，则参考下面代码处理
            List<ExpandoObject> objList = new List<ExpandoObject>();
            foreach (StampUsageInfo info in list)
            {
                dynamic obj = new ExpandoObject();

                obj.ID = info.ID;
                obj.Reason = info.Reason;
                obj.StampType = info.StampType;
                obj.FileName = info.FileName;
                obj.FileCopies = info.FileCopies;
                obj.StampUseDate = info.StampUseDate;
                ...
                //参考转义代码
                //obj.Name = BLLFactory<StampUsage>.Instance.GetNameByID(info.ID);
        ...
        public ActionResult FindByApplyId(string applyId)
        {
            ...
            var result = BLLFactory<StampUsage>.Instance.FindByApplyId(applyId);
            return ToJsonContent(result);
        }
            ...
            try
            {
                if (!string.IsNullOrEmpty(ids))
                {
                    ...
                    foreach (string strId in idArray)
                    {
                        ...
                        {
                            var info = BLLFactory<StampUsage>.Instance.FindByID(strId);
                            if (info != null)
                            {
                                //删除关联表单数据，包括业务表单数据
