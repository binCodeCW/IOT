...
{
    public class PurchaseApprovalController : BusinessController<PurchaseApproval, PurchaseApprovalInfo>
    {
        public PurchaseApprovalController() : base()
        {
        }
        ...
        //导入或导出的字段列表   
        string columnString = "申请事由,期望交付日期,物品名称,型号或规格,数量,费用金额,申请单编号,申请单日期,申请部门,备注信息,附件组别ID,创建人,创建时间";
        ...
        public ActionResult GetExcelData(string guid)
        {
            ...
            List<PurchaseApprovalInfo> list = new List<PurchaseApprovalInfo>();
            ...
            if (table != null)
            {
                ...
                foreach (DataRow dr in table.Rows)
                {
                    ...
                    info.Reason = dr["申请事由"].ToString();
                    converted = DateTime.TryParse(dr["期望交付日期"].ToString(), out dt);
                    if (converted && dt > dtDefault)
                    {
                        info.DeliveryDate = dt;
                    }
                    info.ItemName = dr["物品名称"].ToString();
                    info.Specification = dr["型号或规格"].ToString();
                    info.Quantity = dr["数量"].ToString().ToDecimal();
                    info.TotalAmount = dr["费用金额"].ToString().ToDecimal();
        ...
        /// <returns></returns>
        public ActionResult SaveExcelData(List<PurchaseApprovalInfo> list)
        {
            CommonResult result = new CommonResult();
            if (list != null && list.Count > 0)
            {
                ...
                var trans = BLLFactory<PurchaseApproval>.Instance.CreateTransaction();
                if (trans != null)
                {
                    try
                    {
                        //int seq = 1;
                        foreach (PurchaseApprovalInfo detail in list)
                        {
                            //detail.Seq = seq++;//增加1
                            /*
							...
							*/
                            ...
                            BLLFactory<PurchaseApproval>.Instance.Insert(detail, trans);
                        }
                        trans.Commit();
                        result.Success = true;
                    }
        ...
        public ActionResult Export()
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                dr = datatable.NewRow();
                dr["序号"] = j++;
                dr["申请事由"] = list[i].Reason;
                dr["期望交付日期"] = list[i].DeliveryDate;
                dr["物品名称"] = list[i].ItemName;
                dr["型号或规格"] = list[i].Specification;
                dr["数量"] = list[i].Quantity;
                dr["费用金额"] = list[i].TotalAmount;
            ...
            string filePath = string.Format("/GenerateFiles/{0}/PurchaseApproval.xls", CurrentUser.Name);
        ...
        #region 写入数据前修改部分属性
        protected override void OnBeforeInsert(PurchaseApprovalInfo info)
        {
        ...
        protected override void OnBeforeUpdate(PurchaseApprovalInfo info)
        {
            //子类对参数对象进行修改
            info.Editor = CurrentUser.ID.ToString();
            info.EditTime = DateTime.Now;
        ...
        public override ActionResult FindWithPager()
        {
            ...
            List<PurchaseApprovalInfo> list = null;
        ...
        /// <returns></returns>
        private List<ExpandoObject> ConvertObjectList(List<PurchaseApprovalInfo> list)
        {
            //如果需要修改字段显示，则参考下面代码处理
            List<ExpandoObject> objList = new List<ExpandoObject>();
            foreach (PurchaseApprovalInfo info in list)
            {
                dynamic obj = new ExpandoObject();

                obj.ID = info.ID;
                obj.Reason = info.Reason;
                obj.DeliveryDate = info.DeliveryDate;
                obj.ItemName = info.ItemName;
                obj.Specification = info.Specification;
                ...
                //参考转义代码
                //obj.Name = BLLFactory<PurchaseApproval>.Instance.GetNameByID(info.ID);
        ...
        public ActionResult FindByApplyId(string applyId)
        {
            ...
            var result = BLLFactory<PurchaseApproval>.Instance.FindByApplyId(applyId);
            return ToJsonContent(result);
        }
            ...
            try
            {
                if (!string.IsNullOrEmpty(ids))
                {
                    ...
                    foreach (string strId in idArray)
                    {
                        ...
                        {
                            var info = BLLFactory<PurchaseApproval>.Instance.FindByID(strId);
                            if (info != null)
                            {
                                //删除关联表单数据，包括业务表单数据
