...
{
    public class MaintenanceController : BusinessController<Maintenance, MaintenanceInfo>
    {
        public MaintenanceController() : base()
        {
        }
        ...
        //导入或导出的字段列表   
        string columnString = "故障设备名称,故障描述,报修日期,预计维修费用,申请单编号,申请单日期,申请部门,备注信息,附件组别ID,创建人,创建时间";
        ...
        public ActionResult GetExcelData(string guid)
        {
            ...
            List<MaintenanceInfo> list = new List<MaintenanceInfo>();
            ...
            if (table != null)
            {
                ...
                foreach (DataRow dr in table.Rows)
                {
                    ...
                    info.DeviceName = dr["故障设备名称"].ToString();
                    info.FaultDescription = dr["故障描述"].ToString();
                    converted = DateTime.TryParse(dr["报修日期"].ToString(), out dt);
                    if (converted && dt > dtDefault)
                    {
                        info.RepairDate = dt;
                    }
                    info.RepairFee = dr["预计维修费用"].ToString().ToDecimal();
        ...
        /// <returns></returns>
        public ActionResult SaveExcelData(List<MaintenanceInfo> list)
        {
            CommonResult result = new CommonResult();
            if (list != null && list.Count > 0)
            {
                ...
                var trans = BLLFactory<Maintenance>.Instance.CreateTransaction();
                if (trans != null)
                {
                    try
                    {
                        //int seq = 1;
                        foreach (MaintenanceInfo detail in list)
                        {
                            //detail.Seq = seq++;//增加1
                            /*
							...
							*/
                            ...
                            BLLFactory<Maintenance>.Instance.Insert(detail, trans);
                        }
                        trans.Commit();
                        result.Success = true;
                    }
        ...
        public ActionResult Export()
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                dr = datatable.NewRow();
                dr["序号"] = j++;
                dr["故障设备名称"] = list[i].DeviceName;
                dr["故障描述"] = list[i].FaultDescription;
                dr["报修日期"] = list[i].RepairDate;
                dr["预计维修费用"] = list[i].RepairFee;
            ...
            string filePath = string.Format("/GenerateFiles/{0}/Maintenance.xls", CurrentUser.Name);
        ...
        #region 写入数据前修改部分属性
        protected override void OnBeforeInsert(MaintenanceInfo info)
        {
        ...
        protected override void OnBeforeUpdate(MaintenanceInfo info)
        {
            //子类对参数对象进行修改
            info.Editor = CurrentUser.ID.ToString();
            info.EditTime = DateTime.Now;
        ...
        public override ActionResult FindWithPager()
        {
            ...
            List<MaintenanceInfo> list = null;
        ...
        /// <returns></returns>
        private List<ExpandoObject> ConvertObjectList(List<MaintenanceInfo> list)
        {
            //如果需要修改字段显示，则参考下面代码处理
            List<ExpandoObject> objList = new List<ExpandoObject>();
            foreach (MaintenanceInfo info in list)
            {
                dynamic obj = new ExpandoObject();

                obj.ID = info.ID;
                obj.DeviceName = info.DeviceName;
                obj.FaultDescription = info.FaultDescription;
                obj.RepairDate = info.RepairDate;
                obj.RepairFee = info.RepairFee;
                ...
                //参考转义代码
                //obj.Name = BLLFactory<Maintenance>.Instance.GetNameByID(info.ID);
        ...
        public ActionResult FindByApplyId(string applyId)
        {
            ...
            var result = BLLFactory<Maintenance>.Instance.FindByApplyId(applyId);
            return ToJsonContent(result);
        }
        /// <summary>
        /// 删除多个ID的记录
            ...
            try
            {
                if (!string.IsNullOrEmpty(ids))
                {
                    ...
                    foreach (string strId in idArray)
                    {
                        ...
                        {
                            var info = BLLFactory<Maintenance>.Instance.FindByID(strId);
                            if (info != null)
                            {
                                //删除关联表单数据，包括业务表单数据
