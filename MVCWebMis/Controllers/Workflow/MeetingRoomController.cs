...
{
    public class MeetingRoomController : BusinessController<MeetingRoom, MeetingRoomInfo>
    {
        public MeetingRoomController() : base()
        {
        }
        ...
        //导入或导出的字段列表   
        string columnString = "会议室名称,开始时间,结束时间,时长,参会人数,参会人,申请单编号,申请单日期,申请部门,备注信息,附件组别ID,创建人,创建时间";
        ...
        public ActionResult GetExcelData(string guid)
        {
            ...
            List<MeetingRoomInfo> list = new List<MeetingRoomInfo>();
            ...
            if (table != null)
            {
                ...
                foreach (DataRow dr in table.Rows)
                {
                    ...
                    info.RoomName = dr["会议室名称"].ToString();
                    ...
                    info.Duration = dr["时长"].ToString();
                    info.MeetingPersonNumber = dr["参会人数"].ToString().ToInt32();
                    info.MeetingPerson = dr["参会人"].ToString();
        ...
        /// <returns></returns>
        public ActionResult SaveExcelData(List<MeetingRoomInfo> list)
        {
            CommonResult result = new CommonResult();
            if (list != null && list.Count > 0)
            {
                ...
                var trans = BLLFactory<MeetingRoom>.Instance.CreateTransaction();
                if (trans != null)
                {
                    try
                    {
                        //int seq = 1;
                        foreach (MeetingRoomInfo detail in list)
                        {
                            //detail.Seq = seq++;//增加1
                            /*
							...
							*/
                            ...
                            BLLFactory<MeetingRoom>.Instance.Insert(detail, trans);
                        }
                        trans.Commit();
                        result.Success = true;
                    }
        ...
        public ActionResult Export()
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                dr = datatable.NewRow();
                dr["序号"] = j++;
                dr["会议室名称"] = list[i].RoomName;
                dr["开始时间"] = list[i].StartTime;
                dr["结束时间"] = list[i].EndTime;
                dr["时长"] = list[i].Duration;
                dr["参会人数"] = list[i].MeetingPersonNumber;
                dr["参会人"] = list[i].MeetingPerson;
            ...
            string filePath = string.Format("/GenerateFiles/{0}/MeetingRoom.xls", CurrentUser.Name);
        ...
        #region 写入数据前修改部分属性
        protected override void OnBeforeInsert(MeetingRoomInfo info)
        {
        ...
        protected override void OnBeforeUpdate(MeetingRoomInfo info)
        {
            //子类对参数对象进行修改
            info.Editor = CurrentUser.ID.ToString();
            info.EditTime = DateTime.Now;
        ...
        public override ActionResult FindWithPager()
        {
            ...
            List<MeetingRoomInfo> list = null;
        ...
        /// <returns></returns>
        private List<ExpandoObject> ConvertObjectList(List<MeetingRoomInfo> list)
        {
            //如果需要修改字段显示，则参考下面代码处理
            List<ExpandoObject> objList = new List<ExpandoObject>();
            foreach (MeetingRoomInfo info in list)
            {
                ...
                obj.MeetingPersonNumber = info.MeetingPersonNumber;
                obj.MeetingPerson = info.MeetingPerson;
                ...
                //参考转义代码
                //obj.Name = BLLFactory<MeetingRoom>.Instance.GetNameByID(info.ID);
        ...
        public ActionResult FindByApplyId(string applyId)
        {
            ...
            var result = BLLFactory<MeetingRoom>.Instance.FindByApplyId(applyId);
            return ToJsonContent(result);
        }
        /// <summary>
        /// 删除多个ID的记录
            ...
            try
            {
                if (!string.IsNullOrEmpty(ids))
                {
                    ...
                    foreach (string strId in idArray)
                    {
                        ...
                        {
                            var info = BLLFactory<MeetingRoom>.Instance.FindByID(strId);
                            if (info != null)
                            {
                                //删除关联表单数据，包括业务表单数据
