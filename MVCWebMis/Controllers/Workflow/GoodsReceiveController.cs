...
{
    public class GoodsReceiveController : BusinessController<GoodsReceive, GoodsReceiveInfo>
    {
        #region 导入Excel数据操作

        //导入或导出的字段列表   
        string columnString = "用途,物品名称,数量,申请单编号,申请单日期,申请部门,备注信息,附件组别ID,创建人,创建时间";
        ...
        public ActionResult CheckExcelColumns(string guid)
        {
            ...
            try
            {
                var dt = ConvertExcelFileToTable(guid);
                if (dt != null)
                {
                    //检查列表是否包含必须的字段
        ...
        public ActionResult GetExcelData(string guid)
        {
            ...
            List<GoodsReceiveInfo> list = new List<GoodsReceiveInfo>();
        ...
        /// <returns></returns>
        public ActionResult SaveExcelData(List<GoodsReceiveInfo> list)
        {
            CommonResult result = new CommonResult();
            if (list != null && list.Count > 0)
            {
                ...
                var trans = BLLFactory<GoodsReceive>.Instance.CreateTransaction();
                if (trans != null)
                {
                    try
                    {
                        //int seq = 1;
                        foreach (GoodsReceiveInfo detail in list)
                        {
                            //detail.Seq = seq++;//增加1
                            /*
							...
							*/
                            ...
                            BLLFactory<GoodsReceive>.Instance.Insert(detail, trans);
                        }
                        trans.Commit();
                        result.Success = true;
                    }
        ...
        public ActionResult Export()
        {
            ...
            string filePath = string.Format("/GenerateFiles/{0}/GoodsReceive.xls", CurrentUser.Name);
        ...
        #region 写入数据前修改部分属性
        protected override void OnBeforeInsert(GoodsReceiveInfo info)
        {
        ...
        protected override void OnBeforeUpdate(GoodsReceiveInfo info)
        {
            //子类对参数对象进行修改
            info.Editor = CurrentUser.ID.ToString();
            info.EditTime = DateTime.Now;
        ...
        public override ActionResult FindWithPager()
        {
            ...
            List<GoodsReceiveInfo> list = null;
        ...
        /// <returns></returns>
        private List<ExpandoObject> ConvertObjectList(List<GoodsReceiveInfo> list)
        {
            //如果需要修改字段显示，则参考下面代码处理
            List<ExpandoObject> objList = new List<ExpandoObject>();
            foreach (GoodsReceiveInfo info in list)
            {
        ...
        public ActionResult FindByApplyId(string applyId)
        {
            ...
            var result = BLLFactory<GoodsReceive>.Instance.FindByApplyId(applyId);
            return ToJsonContent(result);
        }
        /// <summary>
        /// 删除多个ID的记录
            ...
            try
            {
                if (!string.IsNullOrEmpty(ids))
                {
                    ...
                    foreach (string strId in idArray)
                    {
                        ...
                        {
                            var info = BLLFactory<GoodsReceive>.Instance.FindByID(strId);
                            if (info != null)
                            {
                                //删除关联表单数据，包括业务表单数据
