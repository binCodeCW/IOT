...
{
    public class LeaveController : BusinessController<Leave, LeaveInfo>
    {
        public LeaveController() : base()
        {
        }
        ...
        //导入或导出的字段列表   
        string columnString = "请假类型,开始时间,结束时间,请假时长,请假事由,附件组别ID,申请单编号,申请单日期,申请部门,备注信息,创建人,创建时间";
        ...
        public ActionResult GetExcelData(string guid)
        {
            ...
            if (table != null)
            {
                ...
                foreach (DataRow dr in table.Rows)
                {
                    ...
                    info.Category = dr["请假类型"].ToString();
                    ...
                    info.Duration = dr["请假时长"].ToString().ToDouble();
                    info.Reason = dr["请假事由"].ToString();
        ...
        /// <returns></returns>
        public ActionResult SaveExcelData(List<LeaveInfo> list)
        {
            CommonResult result = new CommonResult();
            if (list != null && list.Count > 0)
            {
                ...
                var trans = BLLFactory<Leave>.Instance.CreateTransaction();
                if (trans != null)
                {
                    try
                    {
                        //int seq = 1;
                        foreach (LeaveInfo detail in list)
                        {
                            //detail.Seq = seq++;//增加1
                            /*
							...
							*/
                            ...
                            BLLFactory<Leave>.Instance.Insert(detail, trans);
                        }
                        trans.Commit();
                        result.Success = true;
                    }
        ...
        public ActionResult Export()
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                dr = datatable.NewRow();
                dr["序号"] = j++;
                dr["请假类型"] = list[i].Category;
                dr["开始时间"] = list[i].StartTime;
                dr["结束时间"] = list[i].EndTime;
                dr["请假时长"] = list[i].Duration;
                dr["请假事由"] = list[i].Reason;
            ...
            string filePath = string.Format("/GenerateFiles/{0}/Leave.xls", CurrentUser.Name);
        ...
        #region 写入数据前修改部分属性
        protected override void OnBeforeInsert(LeaveInfo info)
        {
        ...
        protected override void OnBeforeUpdate(LeaveInfo info)
        {
            //子类对参数对象进行修改
            info.Editor = CurrentUser.ID.ToString();
            info.EditTime = DateTime.Now;
        ...
        /// <returns></returns>
        private List<ExpandoObject> ConvertObjectList(List<LeaveInfo> list)
        {
            //如果需要修改字段显示，则参考下面代码处理
            List<ExpandoObject> objList = new List<ExpandoObject>();
            foreach (LeaveInfo info in list)
            {
                ...
                //参考转义代码
                //obj.Name = BLLFactory<Leave>.Instance.GetNameByID(info.ID);
        ...
        public ActionResult FindByApplyId(string applyId)
        {
            ...
            var result =  BLLFactory<Leave>.Instance.FindByApplyId(applyId);
            return ToJsonContent(result);
        }
        /// <summary>
        /// 删除多个ID的记录
            ...
            try
            {
                if (!string.IsNullOrEmpty(ids))
                {
                    ...
                    foreach (string strId in idArray)
                    {
                        ...
                        {
                            var info = BLLFactory<Leave>.Instance.FindByID(strId);
                            if (info != null)
                            {
                                //删除关联表单数据，包括业务表单数据
