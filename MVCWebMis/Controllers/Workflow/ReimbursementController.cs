...
{
    public class ReimbursementController : BusinessController<Reimbursement, ReimbursementInfo>
    {
        public ReimbursementController() : base()
        {
        }
        ...
        //导入或导出的字段列表   
        string columnString = "报销类型,报销事由,总金额,申请单编号,申请单日期,申请部门,备注信息,附件组别ID,创建人,创建时间";
        ...
        public ActionResult GetExcelData(string guid)
        {
            ...
            List<ReimbursementInfo> list = new List<ReimbursementInfo>();
            ...
            if (table != null)
            {
                ...
                foreach (DataRow dr in table.Rows)
                {
                    ...
                    info.Category = dr["报销类型"].ToString();
                    info.Reason = dr["报销事由"].ToString();
                    info.TotalAmount = dr["总金额"].ToString().ToDecimal();
        ...
        public ActionResult Export()
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                dr = datatable.NewRow();
                dr["序号"] = j++;
                dr["报销类型"] = list[i].Category;
                dr["报销事由"] = list[i].Reason;
                dr["总金额"] = list[i].TotalAmount;
            ...
            string filePath = string.Format("/GenerateFiles/{0}/Reimbursement.xls", CurrentUser.Name);
        ...
        #region 写入数据前修改部分属性
        protected override void OnBeforeInsert(ReimbursementInfo info)
        {
        ...
        protected override void OnBeforeUpdate(ReimbursementInfo info)
        {
            //子类对参数对象进行修改
            info.Editor = CurrentUser.ID.ToString();
            info.EditTime = DateTime.Now;
        ...
        public override ActionResult FindWithPager()
        {
            ...
            List<ReimbursementInfo> list = null;
        ...
        /// <returns></returns>
        private List<ExpandoObject> ConvertObjectList(List<ReimbursementInfo> list)
        {
            //如果需要修改字段显示，则参考下面代码处理
            List<ExpandoObject> objList = new List<ExpandoObject>();
            foreach (ReimbursementInfo info in list)
            {
                ...
                //参考转义代码
                //obj.Name = BLLFactory<Reimbursement>.Instance.GetNameByID(info.ID);
        ...
        public ActionResult FindByApplyId(string applyId)
        {
            ...
            var result = BLLFactory<Reimbursement>.Instance.FindByApplyId(applyId);
            return ToJsonContent(result);
            ...
            try
            {
                if (!string.IsNullOrEmpty(ids))
                {
                    ...
                    foreach (string strId in idArray)
                    {
                        ...
                        {
                            var info = BLLFactory<Reimbursement>.Instance.FindByID(strId);
                            if (info != null)
                            {
                                //删除关联表单数据，包括业务表单数据
