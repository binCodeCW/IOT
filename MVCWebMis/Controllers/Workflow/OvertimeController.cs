...
{
    public class OvertimeController : BusinessController<Overtime, OvertimeInfo>
    {
        public OvertimeController() : base()
        {
        }
        ...
        //导入或导出的字段列表   
        string columnString = "加班事由,开始时间,结束时间,加班时长-天,加班时长-小时,加班时长,附件组别ID,申请单编号,申请单日期,申请部门,备注信息,创建人,创建时间";
        ...
        public ActionResult GetExcelData(string guid)
        {
            ...
            if (table != null)
            {
                ...
                foreach (DataRow dr in table.Rows)
                {
                    ...
                    info.DurationDay = dr["加班时长-天"].ToString().ToDouble();
                    info.DurationHour = dr["加班时长-小时"].ToString().ToDouble();
                    info.Duration = dr["加班时长"].ToString();
        ...
        /// <returns></returns>
        public ActionResult SaveExcelData(List<OvertimeInfo> list)
        {
            CommonResult result = new CommonResult();
            if (list != null && list.Count > 0)
            {
                ...
                var trans = BLLFactory<Overtime>.Instance.CreateTransaction();
                if (trans != null)
                {
                    try
                    {
                        //int seq = 1;
                        foreach (OvertimeInfo detail in list)
                        {
                            //detail.Seq = seq++;//增加1
                            /*
							...
							*/
                            ...
                            BLLFactory<Overtime>.Instance.Insert(detail, trans);
                        }
                        trans.Commit();
                        result.Success = true;
                    }
        ...
        #region 写入数据前修改部分属性
        protected override void OnBeforeInsert(OvertimeInfo info)
        {
        ...
        protected override void OnBeforeUpdate(OvertimeInfo info)
        {
            //子类对参数对象进行修改
            info.Editor = CurrentUser.ID.ToString();
            info.EditTime = DateTime.Now;
        ...
        /// <returns></returns>
        private List<ExpandoObject> ConvertObjectList(List<OvertimeInfo> list)
        {
            //如果需要修改字段显示，则参考下面代码处理
            List<ExpandoObject> objList = new List<ExpandoObject>();
            foreach (OvertimeInfo info in list)
            {
                ...
                obj.DurationDay = info.DurationDay;
                obj.DurationHour = info.DurationHour;
                ...
                //参考转义代码
                //obj.Name = BLLFactory<Overtime>.Instance.GetNameByID(info.ID);
        ...
        public ActionResult FindByApplyId(string applyId)
        {
            ...
            var result = BLLFactory<Overtime>.Instance.FindByApplyId(applyId);
            return ToJsonContent(result);
        }
            ...
            try
            {
                if (!string.IsNullOrEmpty(ids))
                {
                    ...
                    foreach (string strId in idArray)
                    {
                        ...
                        {
                            var info = BLLFactory<Overtime>.Instance.FindByID(strId);
                            if (info != null)
                            {
                                //删除关联表单数据，包括业务表单数据
