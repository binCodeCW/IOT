...
{
    public class YH01_DeviceInfoRecController : BusinessController<YH01_DeviceInfoRec, YH01_DeviceInfoRecInfo>
    {
        public YH01_DeviceInfoRecController() : base()
        {
        }
        ...
        //导入或导出的字段列表   
        string columnString = "时间戳,时刻,应用ID,服务ID,设备ID,检测时长,探测效率,本底,自动模式,检测时是否扫码,零位校准方向,零位校准脉冲,记录时间,仪器编号";
        ...
        /// <returns></returns>
        public List<YH01_DeviceInfoRecInfo> GetExcelList(string guid)
        {
            List<YH01_DeviceInfoRecInfo> list = new List<YH01_DeviceInfoRecInfo>();
            ...
            if (table != null)
            {
                ...
                foreach (DataRow dr in table.Rows)
                {
                    ...
                    info.Autoresult = dr["自动模式"].ToString();
                    info.Enscan = dr["检测时是否扫码"].ToString();
                    info.Zpcorrectiondirection = dr["零位校准方向"].ToString();
                    info.Zpcorrectionpulse = dr["零位校准脉冲"].ToString();
        ...
        /// <returns></returns>
        public CommonResult SaveList(List<YH01_DeviceInfoRecInfo> list)
        {
            CommonResult result = new CommonResult();
            if (list != null && list.Count > 0)
            {
                ...
                var trans = BLLFactory<YH01_DeviceInfoRec>.Instance.CreateTransaction();
                if (trans != null)
                {
                    try
                    {
                        foreach (YH01_DeviceInfoRecInfo detail in list)
                        {
                            //修改部分信息
                            OnBeforeInsert(detail);

                            //在此判断插入数据条件
                            //var isExist = BLLFactory<YH01_DeviceInfoRec>.Instance.IsExistKey("Code", detail.Code, trans);
                            //if (!isExist)
                            {
                                BLLFactory<YH01_DeviceInfoRec>.Instance.Insert(detail, trans);
                            }
                        }
                        trans.Commit();
                        result.Success = true;
        ...
        /// <returns></returns>
        public ActionResult SaveExcelData(List<YH01_DeviceInfoRecInfo> list)
        {
        ...
        public ActionResult Export()
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                dr["本底"] = list[i].Bendi;
                dr["自动模式"] = list[i].Autoresult;
                dr["检测时是否扫码"] = list[i].Enscan;
                dr["零位校准方向"] = list[i].Zpcorrectiondirection;
                dr["零位校准脉冲"] = list[i].Zpcorrectionpulse;
            ...
            string filePath = string.Format("/GenerateFiles/{0}/YH01_DeviceInfoRec.xls", CurrentUser.Name);
        ...
        #region 写入数据前修改部分属性
        protected override void OnBeforeInsert(YH01_DeviceInfoRecInfo info)
        {
        ...
        protected override void OnBeforeUpdate(YH01_DeviceInfoRecInfo info)
        {
            //子类对参数对象进行修改
            //info.Editor = CurrentUser.ID.ToString();
            //info.EditTime = DateTime.Now;
        ...
        public ActionResult DeviceInfo1(int? deviceNo, DateTime? startTime, DateTime? endTime)
        {
            ...
            string sql = @"
select * from  YH01_DeviceInfoRec t
where 1 = 1
...
";
        ...
        public override ActionResult FindWithPager()
        {
            ...
            List<YH01_DeviceInfoRecInfo> list = null;
        ...
        /// <returns></returns>
        private List<ExpandoObject> ConvertObjectList(List<YH01_DeviceInfoRecInfo> list)
        {
            //如果需要修改字段显示，则参考下面代码处理
            List<ExpandoObject> objList = new List<ExpandoObject>();
            foreach (YH01_DeviceInfoRecInfo info in list)
            {
                ...
                obj.Autoresult = info.Autoresult;
                obj.Enscan = info.Enscan;
                obj.Zpcorrectiondirection = info.Zpcorrectiondirection;
                obj.Zpcorrectionpulse = info.Zpcorrectionpulse;
                obj.Recordtime = info.Recordtime;
                obj.Deviceno = info.Deviceno;
                //参考转义代码
                //obj.Name = BLLFactory<YH01_DeviceInfoRec>.Instance.GetNameByID(info.ID);
