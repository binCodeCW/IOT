...
namespace IOT.MVCWebMis.Controllers
{
    public class MenuController : BusinessController<Menu, MenuInfo>
    {
        ...
        protected override void OnBeforeInsert(MenuInfo info)
        {
            ...
            info.Creator = CurrentUser.ID.ToString();
            //确保二进制的内容不为空
            if (info.EmbedIcon == null)
            {
                info.EmbedIcon = new byte[] { };
            }
        ...
        public ActionResult GetDictJson()
        {
            ...
            List<MenuInfo> list = baseBLL.Find(where);
        ...
        public ActionResult GetMenuData()
        {
            ...
            List<MenuInfo> list = BLLFactory<Menu>.Instance.GetTopMenu(ConfigData.SystemType);
            int i = 0;
            foreach (MenuInfo info in list)
            {
        ...
        {
            var list = BLLFactory<Menu>.Instance.GetMenusByRole(roleid, ConfigData.SystemType);
