...
namespace IOT.MVCWebMis.Controllers
{
    public class FunctionController : BusinessController<Function, FunctionInfo>
    {
        ...
        //导入或导出的字段列表   
        string columnString = "父ID,功能名称,控制标识,系统编号,排序码";
        ...
        public ActionResult GetExcelData(string guid)
        {
            ...
            if (table != null)
            {
                ...
                foreach (DataRow dr in table.Rows)
                {
                    ...
                    info.ControlID = dr["控制标识"].ToString();
        ...
        public override ActionResult FindWithPager()
        {
            ...
            string where = GetPagerCondition();
            //限定系统类型，不展示其他系统内容
            where += string.Format(" AND SystemType_ID='{0}'", ConfigData.SystemType);
        ...
        {
            string where = string.Format(" SystemType_ID='{0}'", ConfigData.SystemType);
            List<FunctionInfo> comboList = BLLFactory<Function>.Instance.Find(where);
        ...
        /// <returns></returns>
        protected override CommonResult Update(string id, FunctionInfo info)
        {
        ...
        public ActionResult GetFunctionDictJson()
        {
            ...
            CListItem pNode = new CListItem("无", "-1");
            treeList.Add(pNode);

            string condition = string.Format("SystemType_ID='{0}'", ConfigData.SystemType);//系统标识ID
            ...
            {
                treeList.Add(new CListItem(info.Name, info.ID));
            }
        ...
        private void AddDictChildNode(List<FunctionNodeInfo> list, List<CListItem> treeList)
        {
            foreach (FunctionNodeInfo info in list)
            {
                CListItem item = new CListItem(info.Name, info.ID);
                treeList.Add(item);
