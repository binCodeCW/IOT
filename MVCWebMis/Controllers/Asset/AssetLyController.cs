...
{
    public class AssetLyController : BusinessController<AssetLy, AssetLyInfo>
    {
        public AssetLyController() : base()
        {
        }
	    ...
	    //导入或导出的字段列表   
        string columnString = "领用单号,领用资产,资产使用部门（单位）,资产管理部门（单位）,使用部门资产管理员,申请单编号,申请单日期,申请部门,备注信息,附件组别ID,创建人,创建时间";
        ...
        public ActionResult GetExcelData(string guid)
        {
            ...
            List<AssetLyInfo> list = new List<AssetLyInfo>();
        ...
        /// <returns></returns>
        public ActionResult SaveExcelData(List<AssetLyInfo> list)
        {
            CommonResult result = new CommonResult();
            if (list != null && list.Count > 0)
            {
                ...
                var trans = BLLFactory<AssetLy>.Instance.CreateTransaction();
                if (trans != null)
                {
                    try
                    {
                        //int seq = 1;
                        foreach (AssetLyInfo detail in list)
                        {
                            //detail.Seq = seq++;//增加1
							/*
							...
							*/
                            ...
                            BLLFactory<AssetLy>.Instance.Insert(detail, trans);
                        }
                        trans.Commit();
                        result.Success = true;
                    }
        ...
        public ActionResult Export()
        {
            ...
            string filePath = string.Format("/GenerateFiles/{0}/AssetLy.xls", CurrentUser.Name);
        ...
        #region 写入数据前修改部分属性
        protected override void OnBeforeInsert(AssetLyInfo info)
        {
            ...
            //info.ApplyDate = DateTime.Now;//已经在客户端中赋值
            //info.Company_ID = CurrentUser.Company_ID;
            //info.Dept_ID = CurrentUser.Dept_ID;
        }

        protected override void OnBeforeUpdate(AssetLyInfo info)
        {
            //子类对参数对象进行修改
            info.Editor = CurrentUser.ID.ToString();
            info.EditTime = DateTime.Now;
        ...
        /// <returns></returns>
        private List<ExpandoObject> ConvertObjectList(List<AssetLyInfo> list)
        {
			//如果需要修改字段显示，则参考下面代码处理
            List<ExpandoObject> objList = new List<ExpandoObject>();
            foreach (AssetLyInfo info in list)
            {
                ...
                obj.LyDept = SecurityHelper.GetDeptNameByID(info.LyDept);
                ...
                //参考转义代码
                //obj.Name = BLLFactory<AssetLy>.Instance.GetNameByID(info.ID);
        ...
        public ActionResult GetBillNo()
        {
            ...
            //conditoin.AddDateCondition("CreateTime", DateTime.Now.ToString("yyyy-MM-dd"));//2020228
        ...
        public ActionResult SaveApply(JObject param)
        {
            ...
            if (obj != null)
            {
                ...
                {
                    //获取主信息
                    AssetLyInfo info = (JObject.FromObject(obj.info)).ToObject<AssetLyInfo>();

                    //转换为明细信息
                    List<AssetLyDetailInfo> details = null;
                    if (obj.details != null)
                    {
                        details = (JArray.FromObject(obj.details)).ToObject<List<AssetLyDetailInfo>>();
                    }
                    ...
                    if (info != null)
                    {
                        //修改部分信息
                        OnBeforeInsert(info);
                        bool succeed = BLLFactory<AssetLy>.Instance.InsertUpdate(info, info.ID);
                        if (succeed)
                        {
                            //移除已有列表数据，重新加入
                            ...
                            if (details != null)
                            {
                                foreach (var detailInfo in details)
                                {
                                    ...
                                    //客户端提交LyDept和UsePerson需要解析，把Name|ID 转换为Name
                                    //detailInfo.LyDept = HtmlHelpers.GetSplitName(detailInfo.LyDept);
                                    //detailInfo.UsePerson = HtmlHelpers.GetSplitName(detailInfo.UsePerson);

                                    BLLFactory<AssetLyDetail>.Instance.InsertUpdate(detailInfo, detailInfo.ID);
                                }
                            }
        ...
        public ActionResult FindByApplyId(string applyId)
        {
            ...
            var result = BLLFactory<AssetLy>.Instance.FindByApplyId(applyId);
            return ToJsonContent(result);
        }
            ...
            try
            {
                if (!string.IsNullOrEmpty(ids))
                {
                    ...
                    foreach (string strId in idArray)
                    {
                        ...
                        {
                            var info = BLLFactory<AssetLy>.Instance.FindByID(strId);
                            if (info != null)
                            {
                                //由于明细没有进行级联删除，此处删除明细
                                var condition = string.Format("BillNo='{0}'", info.BillNo);
        ...
        public ActionResult ToCompletedForm(string applyId)
        {
            ...
            if(applyInfo != null)
            {
                ...
                {
                    var info = BLLFactory<AssetLy>.Instance.FindByApplyId(applyId);
                    if (info != null)
                    {
                        var list = BLLFactory<AssetLyDetail>.Instance.FindByBillNo(info.BillNo);
                        if(list != null && list.Count > 0)
                        {
                            foreach(var detail in list)
                            {
                                ...
                                if(asset != null)
                                {
                                    //领用成功后，将资产主表T_Asset资产动态 Status 更新为“领用”	,使用状况 UseStatus 更新为“在用”	
                                    //CurrDept/ChargeDept/Operator/OperateDate/UsePerson/KeepAddr/Unit/Qty/OriginValue/Status/UseStatus
                                    asset.CurrDept = SecurityHelper.GetDeptNameByID(info.LyDept);
