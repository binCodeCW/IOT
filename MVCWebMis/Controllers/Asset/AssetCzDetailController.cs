...
{
    public class AssetCzDetailController : BusinessController<AssetCzDetail, AssetCzDetailInfo>
    {
        public AssetCzDetailController() : base()
        {
        }
	    ...
	    //导入或导出的字段列表   
        string columnString = "处置单号,资产编号,资产名称,处置部门,使用人,使用地点,存放地点,处置方式,单位,单价,数量,金额,备注,创建人,创建时间";
        ...
        public ActionResult GetExcelData(string guid)
        {
            ...
            List<AssetCzDetailInfo> list = new List<AssetCzDetailInfo>();
                ...
                foreach (DataRow dr in table.Rows)
                {
                    ...
                    AssetCzDetailInfo info = new AssetCzDetailInfo();
                     ...
                     info.BillNo = dr["处置单号"].ToString();
                      ...
                      info.CzDept = dr["处置部门"].ToString();
                      info.UsePerson = dr["使用人"].ToString();
                      info.UseAddr = dr["使用地点"].ToString();
                      info.KeepAddr = dr["存放地点"].ToString();
                      info.DisposeType = dr["处置方式"].ToString();
        ...
        /// <returns></returns>
        public ActionResult SaveExcelData(List<AssetCzDetailInfo> list)
        {
            CommonResult result = new CommonResult();
            if (list != null && list.Count > 0)
            {
                ...
                var trans = BLLFactory<AssetCzDetail>.Instance.CreateTransaction();
                if (trans != null)
                {
                    try
                    {
                        //int seq = 1;
                        foreach (AssetCzDetailInfo detail in list)
                        {
                            //detail.Seq = seq++;//增加1
							/*
							...
							*/
                            ...
                            BLLFactory<AssetCzDetail>.Instance.Insert(detail, trans);
                        }
                        trans.Commit();
                        result.Success = true;
                    }
        ...
        public ActionResult Export()
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                dr["序号"] = j++;                
                 ...
                 dr["处置部门"] = list[i].CzDept;
                 dr["使用人"] = list[i].UsePerson;
                 dr["使用地点"] = list[i].UseAddr;
                 dr["存放地点"] = list[i].KeepAddr;
                 dr["处置方式"] = list[i].DisposeType;
                 dr["单位"] = list[i].Unit;
                 dr["单价"] = list[i].Price;
            ...
            string filePath = string.Format("/GenerateFiles/{0}/AssetCzDetail.xls", CurrentUser.Name);
        ...
        #region 写入数据前修改部分属性
        protected override void OnBeforeInsert(AssetCzDetailInfo info)
        {
        ...
        protected override void OnBeforeUpdate(AssetCzDetailInfo info)
        {
            //子类对参数对象进行修改
            //info.Editor = CurrentUser.ID.ToString();
            //info.EditTime = DateTime.Now;
        ...
        public override ActionResult FindWithPager()
        {
            ...
            List<AssetCzDetailInfo> list = null;
        ...
        /// <returns></returns>
        private List<ExpandoObject> ConvertObjectList(List<AssetCzDetailInfo> list)
        {
			//如果需要修改字段显示，则参考下面代码处理
            List<ExpandoObject> objList = new List<ExpandoObject>();
            foreach(AssetCzDetailInfo info in list)
            {
                ...
                obj.ID = info.ID;
                 ...
                 obj.CzDept = info.CzDept;
                 obj.UsePerson = info.UsePerson;
                 obj.UseAddr = info.UseAddr;
                 obj.KeepAddr = info.KeepAddr;
                 obj.DisposeType = info.DisposeType;
                 obj.Unit = info.Unit;
 				...
 				//参考转义代码
				//obj.Name = BLLFactory<AssetCzDetail>.Instance.GetNameByID(info.ID);
        ...
        public ActionResult FindByBillNo(string billNo)
        {
            ...
            var result = BLLFactory<AssetCzDetail>.Instance.FindByBillNo(billNo);
            return ToJsonContent(result);
        }
    }
}
