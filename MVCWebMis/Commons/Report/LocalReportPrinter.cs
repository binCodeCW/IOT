...
namespace IOT.MVCWebMis.Common
{
    ...
    public class LocalReportPrinter
    {
        ...
        internal PageSettings PageSettings
        {
            get
            {
                PageSettings pageSettings = this._pageSettings;
                if (pageSettings == null)
                {
                    ReportPageSettings reportPageSettings = this._report.GetDefaultPageSettings();
                    ...
                    pageSettings = func(this.PrinterSettings);
        ...
        internal LocalReportPrinter(LocalReport report)
        {
            _report = report;
            _fileManagerWrapper = new FileManagerWrapper();
            this._processingThread = new ProcessingThread();
        }
        ...
        {
            Assembly asm = typeof(LocalReport).Assembly;
        ...
        {
            return this._processingThread.Cancel(millisecondsTimeout);
        }
        ...
        public void Print()
        {
            this.Print(this.CreateDefaultPrintSettings());
        }
        ...
        public void Print(PrinterSettings printerSettings)
        {
            using (PrintDialog dialog = new PrintDialog())
            {
                ...
                if (fileManagerWrapper.Status == FileManagerStatus.Aborted || fileManagerWrapper.Status == FileManagerStatus.NotStarted)
                {
                    ...
                    this.BeginAsyncRender("IMAGE",
                        true,
                        deviceInfo,
                        PageCountMode.Estimate,
                        new CreateAndRegisterStream(this.CreateStreamEMFPrintOnly),
                        new AsyncCompletedEventHandler(this.OnRenderingCompletePrintOnly),
                        !printAllPages);
                }
                ...
                PrintDocument document = (PrintDocument)Activator.CreateInstance(
                    _reportPrintDocumentType,
                     ...
                     null,
                     new object[] { fileManagerWrapper.FileManager, this.PageSettings.Clone() },
                     null);
        ...
        {
            OnPrintComplete(this._fileManagerWrapper.Status);
        }
...
"</DeviceInfo>", startPage, endPage, str);
        ...
        {
            LocalReport report = this._report;
        ...
        private void OnRenderingCompletePrintOnly(object sender, AsyncCompletedEventArgs args)
        {
            ...
            {
                this._fileManagerWrapper.Status = FileManagerStatus.Aborted;
            }
            else
            {
                this._fileManagerWrapper.Status = FileManagerStatus.Complete;
            }
        }
    }
    /// <summary>
