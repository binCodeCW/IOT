...
namespace WHC.CRM.DALSQL
{
    ...
    public class Customer : BaseDALSQL<CustomerInfo>, ICustomer
    {
        ...
        {
            string sql = string.Format("Update {0} Set Creator='{1}', Company_ID='{2}' Where ID = '{3}' ", tableName, userId, companyId, id);
        ...
        {
            //所属用户条件,非删除状态
            string where = "";// string.Format(" AND Deleted=0");
            ...
            {
                sql = string.Format("Select t.* from {0} t where ID not In (select Customer_ID from T_CRM_CustomerGroup_Customer) {1}", tableName, where);
            }
            else
            {
                ...
                where m.CustomerGroup_ID in ({1}) {2} ", tableName, subSql, where);
            }
